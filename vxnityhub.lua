    ...
    function(value)
        if value then
            ...
            HideReachConnection = RunService.RenderStepped:Connect(function()
                ...
                if not FakeBall then
                    FakeBall = realBall:Clone()
                    FakeBall.Anchored = true
                    FakeBall.CanCollide = false
                    FakeBall.Parent = workspace

                    for _, v in FakeBall:GetDescendants() do
                ...
                FakeBall.CFrame = hrp.CFrame * CFrame.new(0, -2.6, -1.2)
            end)
        else
...
local AvatarSection = createSection(TabMisc, "Avatar Preview")
...
local FakeLevelSection = createSection(TabMisc, "Level")
...
addButton(
    ...
    function()
        ...
        if PlayerGui:FindFirstChild("LevelGui") then
            PlayerGui.FakeLevelGui:Destroy()
        end
        ...
        box.FocusLost:Connect(function(enter)
            ...
            local levelStat =
                leaderstats:FindFirstChild("Level") or
                leaderstats:FindFirstChild("PPLevel")
